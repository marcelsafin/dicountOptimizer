...
class Timeframe(BaseModel):
    ...
    Example:
        >>> from datetime import date
        >>> timeframe = Timeframe(start_date=date(2025, 11, 14), end_date=date(2025, 11, 21))
    """
...
class DiscountItem(BaseModel):
    ...
    original_price: Annotated[
        Decimal, Field(gt=0, decimal_places=2, description="Original price before discount")
    ]
    discount_price: Annotated[
        Decimal, Field(gt=0, decimal_places=2, description="Discounted price")
    ]
    ...
    travel_distance_km: Annotated[
        float, Field(ge=0, description="Distance to store in kilometers")
    ] = 0.0
    travel_time_minutes: Annotated[
        float, Field(ge=0, description="Estimated travel time in minutes")
    ] = 0.0
...
class ShoppingRecommendation(BaseModel):
    ...
    total_savings: Annotated[
        Decimal, Field(ge=0, decimal_places=2, description="Total amount saved")
    ]
