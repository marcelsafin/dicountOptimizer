...
class AgentFactory:
    ...
    def _validate_configuration(self) -> None:
        ...
        if self.config.agent_temperature < 0.0 or self.config.agent_temperature > 2.0:
            errors.append(f"AGENT_TEMPERATURE must be between 0.0 and 2.0, got {self.config.agent_temperature}")
        ...
        if self.config.cache_ttl_seconds <= 0:
            errors.append(f"CACHE_TTL_SECONDS must be positive, got {self.config.cache_ttl_seconds}")
        
        if self.config.api_timeout_seconds <= 0:
            errors.append(f"API_TIMEOUT_SECONDS must be positive, got {self.config.api_timeout_seconds}")
    ...
    def get_input_validation_service(self) -> InputValidationService:
        ...
        if self._input_validator is None:
            ...
            self._input_validator = InputValidationService(
