...
from ..domain.models import DiscountItem, Purchase, OptimizationPreferences, Location
...
class OptimizationInput(BaseModel):
    ...
    preferences: OptimizationPreferences = Field(
        description="User's optimization preferences"
    )
    user_location: Location = Field(
        description="User's geographic location for distance calculations"
    )
    timeframe_start: date = Field(
        description="Start date of shopping timeframe"
    )
    timeframe_end: date = Field(
        description="End date of shopping timeframe"
    )
...
class OptimizationOutput(BaseModel):
    ...
    purchases: list[Purchase] = Field(
        description="List of optimized purchase recommendations"
    )
    total_savings: Decimal = Field(
        ge=0,
        description="Total amount saved across all purchases"
    )
    total_items: int = Field(
        ge=0,
        description="Total number of items to purchase"
    )
    unique_stores: int = Field(
        ge=0,
        description="Number of unique stores to visit"
    )
    store_summary: dict[str, int] = Field(
        description="Summary of items per store"
    )
    optimization_notes: str = Field(
        default="",
        description="Notes about optimization decisions"
    )
...
class MultiCriteriaOptimizerService:
    ...
    def _optimize_purchases(
        ...
        logger.debug(
            "starting_optimization_algorithm",
            num_ingredients=len(input_data.ingredient_matches)
        )
        ...
        for ingredient, discount_item in final_selections.items():
            ...
            purchase_day = self._calculate_optimal_purchase_day(
                discount_item.expiration_date,
                input_data.timeframe_start
            )
    ...
    def _calculate_score(
        ...
        active_preferences = sum([
            preferences.maximize_savings,
            preferences.minimize_stores,
            preferences.prefer_organic
        ])
    ...
    def _calculate_optimal_purchase_day(
        self,
        expiration_date: date,
    ...
    def _generate_optimization_notes(
        ...
        if num_stores == 1:
            ...
            notes_parts.append(f"Shopping consolidated to {num_stores} stores for good balance of savings and convenience")
        else:
