...
from agents.discount_optimizer.domain.protocols import CacheRepository, DiscountRepository
...
class DiscountMatchingInput(BaseModel):
    ...
    radius_km: float = Field(
        default=5.0, gt=0.0, le=100.0, description="Search radius in kilometers"
    )
    ...
    prefer_organic: bool = Field(
        default=False, description="Whether to prioritize organic products"
    )
    max_results: int = Field(
        default=100, gt=0, le=500, description="Maximum number of discount items to return"
    )
...
class DiscountMatchingOutput(BaseModel):
    ...
    cache_hit: bool = Field(
        default=False, description="Whether the result was retrieved from cache"
    )
    organic_count: int = Field(ge=0, description="Number of organic products in results")
    average_discount_percent: float = Field(
        ge=0.0, le=100.0, description="Average discount percentage across all results"
    )
...
class DiscountMatcherService:
    ...
    async def match_discounts(self, input_data: DiscountMatchingInput) -> DiscountMatchingOutput:
        ...
        try:
            ...
            logger.exception("discount_fetch_failed", error=str(e))
            raise
        except Exception as e:
            logger.exception("discount_matching_failed", error=str(e), error_type=type(e).__name__)
            return self._fallback_results(input_data)

    def _apply_filters(
        self, discounts: list[DiscountItem], input_data: DiscountMatchingInput
    ) -> list[DiscountItem]:
        ...
        logger.debug(
            "discount_percent_filter_applied",
            min_percent=input_data.min_discount_percent,
    ...
    async def _get_from_cache(self, cache_key: str) -> DiscountMatchingOutput | None:
        ...
        try:
            ...
            logger.warning(
                "invalid_cache_data_type",
                expected="DiscountMatchingOutput",
                actual=type(cached_result).__name__,
            )
            return None
        except Exception as e:
    ...
    async def _save_to_cache(self, cache_key: str, output: DiscountMatchingOutput) -> None:
        ...
        try:
            ...
            await self.cache_repository.set(
                cache_key, cached_data, ttl_seconds=settings.cache_ttl_seconds
            )
            logger.debug(
                "discount_result_cached",
                cache_key=cache_key,
