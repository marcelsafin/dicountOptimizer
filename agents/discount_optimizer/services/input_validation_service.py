...
from agents.discount_optimizer.domain.models import Location, OptimizationPreferences, Timeframe
from agents.discount_optimizer.domain.protocols import GeocodingService
...
class ValidationInput(BaseModel):
    ...
    meal_plan: list[str] = Field(
        default_factory=list, description="List of meal names or descriptions", max_length=20
    )
    timeframe: str = Field(
    ...
    num_meals: int | None = Field(
        default=None, ge=1, le=10, description="Number of meals to suggest"
    )
...
class ValidationOutput(BaseModel):
    ...
    location: Location | None = Field(
        default=None, description="Validated location with coordinates"
    )
    timeframe: Timeframe | None = Field(
        default=None, description="Validated timeframe with start and end dates"
    )
    preferences: OptimizationPreferences | None = Field(
        default=None, description="Validated optimization preferences"
    )
    search_radius_km: float = Field(
        default=5.0, description="Validated search radius in kilometers"
    )
    num_meals: int = Field(default=3, description="Validated number of meals to suggest")
    meal_plan: list[str] = Field(
        default_factory=list, description="Validated meal plan (empty if AI suggestions requested)"
    )
...
class InputValidationService:
    ...
    Example:
        ...
        >>> input_data = ValidationInput(
        ...     address="Nørrebrogade 20, Copenhagen", timeframe="this week", maximize_savings=True
        ... )
    ...
    async def run(self, input_data: ValidationInput) -> ValidationOutput:
        ...
        # Validate location
        location = await self._validate_location(input_data, validation_errors, warnings)

        # Validate timeframe
        timeframe = self._validate_timeframe(input_data.timeframe, validation_errors, warnings)

        # Validate preferences
        preferences = self._validate_preferences(input_data, validation_errors, warnings)

        # Validate search radius
        search_radius_km = self._validate_search_radius(
            input_data.search_radius_km, validation_errors, warnings
        )

        # Validate meal plan
        meal_plan = self._validate_meal_plan(input_data.meal_plan, validation_errors, warnings)

        # Validate number of meals
        num_meals = self._validate_num_meals(
            input_data.num_meals, len(meal_plan), validation_errors, warnings
        )
        ...
        if is_valid:
            logger.info("input_validation_completed", is_valid=True, warnings_count=len(warnings))
        else:
            logger.warning(
    ...
    async def _validate_location(
        self, input_data: ValidationInput, validation_errors: list[str], warnings: list[str]
    ) -> Location | None:
        """
        ...
        if input_data.latitude is not None and input_data.longitude is not None:
            try:
                location = Location(latitude=input_data.latitude, longitude=input_data.longitude)
                logger.debug(
                ...
                validation_errors.append(f"Invalid coordinates: {e!s}")
                return None

        # Check if address is provided
        if input_data.address:
            try:
                location = await self.geocoding_service.geocode_address(input_data.address)
                logger.debug(
                ...
                validation_errors.append(f"Failed to geocode address '{input_data.address}': {e!s}")
                return None
    ...
    def _validate_timeframe(
        self, timeframe_str: str, validation_errors: list[str], warnings: list[str]
    ) -> Timeframe | None:
        """
        ...
        try:
            ...
            validation_errors.append(f"Failed to parse timeframe '{timeframe_str}': {e!s}")
            return None
    ...
    def _validate_preferences(
        ...
        try:
            ...
            validation_errors.append(f"Invalid optimization preferences: {e!s}")
            return None

    def _validate_search_radius(
        self, search_radius_km: float | None, validation_errors: list[str], warnings: list[str]
    ) -> float:
        """
        Validate search radius.
        ...
        if search_radius_km is None:
            ...
            logger.debug("search_radius_defaulted", radius_km=radius)
            return radius

        # Validate range
        if search_radius_km <= 0:
            validation_errors.append(f"Search radius must be positive, got {search_radius_km} km")
            return settings.default_search_radius_km

        if search_radius_km > 50:
            warnings.append(f"Search radius {search_radius_km} km is very large, capping at 50 km")
            return 50.0

        logger.debug("search_radius_validated", radius_km=search_radius_km)

        return search_radius_km

    def _validate_meal_plan(
        self, meal_plan: list[str], validation_errors: list[str], warnings: list[str]
    ) -> list[str]:
        """
        Validate meal plan.
        ...
        for meal in meal_plan:
            ...
            if len(meal_stripped) > 200:
                warnings.append(f"Meal name too long (max 200 chars): '{meal_stripped[:50]}...'")
                continue

            validated_meals.append(meal_stripped)

        if len(validated_meals) > 20:
            warnings.append(f"Meal plan has {len(validated_meals)} meals, limiting to 20 meals")
            validated_meals = validated_meals[:20]

        logger.debug("meal_plan_validated", num_meals=len(validated_meals))
    ...
    def _validate_num_meals(
        ...
        if meal_plan_length > 0:
            logger.debug("num_meals_from_meal_plan", num_meals=meal_plan_length)
            return meal_plan_length

        # If num_meals is provided, validate it
        if num_meals is not None:
            if num_meals < 1:
                validation_errors.append(f"Number of meals must be at least 1, got {num_meals}")
                return 3  # Default

            if num_meals > 10:
                warnings.append(f"Number of meals {num_meals} is very large, capping at 10")
                return 10

            logger.debug("num_meals_validated", num_meals=num_meals)
            return num_meals
