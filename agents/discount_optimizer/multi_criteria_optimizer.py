...
from .models import DiscountItem, Purchase, OptimizationPreferences, Location
...
class MultiCriteriaOptimizer:
    ...
    def calculate_score(
        ...
        store_item_counts: Dict[str, int] = None
    ) -> float:
        """
        ...
        if preferences.maximize_savings:
            savings_score = (purchase_option.original_price - purchase_option.discount_price) / purchase_option.original_price
            score += savings_score * savings_weight
        
        # Calculate distance score: 1 / (1 + distance_km)
        if preferences.minimize_stores:
            distance_km = self.discount_matcher.calculate_distance(user_location, purchase_option.store_location)
    ...
    def optimize(
        ...
        for ingredient, discount_options in matches.items():
            ...
            if best_option:
                ...
                store_item_counts[best_option.store_name] = store_item_counts.get(best_option.store_name, 0) + 1
