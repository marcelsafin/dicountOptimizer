...
class MealSuggestionInput(BaseModel):
    ...
    user_preferences: str = Field(
        default="",
        description="User dietary preferences or meal desires",
        max_length=500
    )
    num_meals: int = Field(
        default=3,
...
class MealSuggestionOutput(BaseModel):
    ...
    suggested_meals: list[str] = Field(
        description="List of suggested meal names",
        min_length=1,
        max_length=10
    )
    reasoning: str = Field(
        description="Explanation of meal suggestions and product usage",
        max_length=1000
    )
    urgency_notes: str = Field(
        default="",
        description="Notes about products expiring soon",
        max_length=500
    )
...
- Family-friendly when possible"""
    ...
    def _parse_response(
        ...
        try:
            ...
            logger.warning(
                "json_parsing_failed_falling_back_to_text_parsing",
                error=str(e)
            )
