...
from ..domain.exceptions import ShoppingOptimizerError, ValidationError, APIError, AgentError
from ..domain.models import Location, Timeframe, OptimizationPreferences, ShoppingRecommendation
from ..logging import get_logger, set_correlation_id, get_correlation_id, LogContext
...
from .output_formatter_agent import OutputFormatterAgent, FormattingInput

# Import services
from ..services.input_validation_service import InputValidationService, ValidationInput
from ..services.discount_matcher_service import DiscountMatcherService, DiscountMatchingInput
from ..services.multi_criteria_optimizer_service import MultiCriteriaOptimizerService, OptimizationInput
...
class ShoppingOptimizerInput(BaseModel):
    ...
    num_meals: int | None = Field(
        ...
        ge=1,
        le=10,
        description="Number of meals to suggest (if meal_plan is empty)"
    )
...
class ShoppingOptimizerAgent:
        ...
        with LogContext(correlation_id=correlation_id, agent='shopping_optimizer'):
            ...
            try:
                ...
                logger.info(
                    ...
                    unique_stores=len(set(d.store_name for d in discount_result.discounts)),
                    correlation_id=correlation_id
                )
                ...
                recommendation = await self._format_output(
                    optimization_result, location
                )
                ...
                raise ShoppingOptimizerError(
                    f"Shopping optimization failed: {str(e)}"
                ) from e
    ...
    async def _validate_input(
        ...
        validation_result = await self.input_validator.run(validation_input)
    ...
    async def _fetch_discounts(
        ...
        try:
            discount_result = await self.discount_matcher.match_discounts(discount_input)
            ...
            from ..services.discount_matcher_service import DiscountMatchingOutput
    ...
    async def _suggest_meals(
        ...
        if not discounts:
            logger.warning(
                "no_discounts_available_using_fallback_meals",
                correlation_id=get_correlation_id()
            )
        ...
        try:
            meal_suggestions = await self.meal_suggester.run(meal_input)
    ...
    async def _optimize_purchases(
        ...
        optimization_result = self.optimizer.optimize(optimization_input)
