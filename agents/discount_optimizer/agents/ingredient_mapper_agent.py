...
class IngredientMapping(BaseModel):
    ...
    best_match: ProductMatch | None = Field(
        default=None,
        description="Highest-scoring product match"
    )
...
class IngredientMappingInput(BaseModel):
    ...
    ingredients: list[str] = Field(
        description="List of ingredient names required for the meal",
        min_length=1,
        max_length=50
    )
...
class IngredientMappingOutput(BaseModel):
    ...
    mappings: list[IngredientMapping] = Field(
        description="List of ingredient mappings"
    )
    total_ingredients: int = Field(
        ge=0,
        description="Total number of ingredients"
    )
    ingredients_with_matches: int = Field(
        ge=0,
        description="Number of ingredients with matches"
    )
    coverage_percent: float = Field(
        ge=0.0,
        le=100.0,
        description="Percentage of ingredients matched"
    )
    unmapped_ingredients: list[str] = Field(
        default_factory=list,
        description="Ingredients with no matches"
    )
...
class IngredientMapperAgent:
    ...
    Example:
        ...
        >>> print(f"Matched {output.ingredients_with_matches}/{output.total_ingredients} ingredients")
        ...
        try:
            ...
            for ingredient in input_data.ingredients:
                ...
                ingredient_data = next(
                    (m for m in mappings_data if m.get('ingredient', '').lower() == ingredient.lower()),
                    None
                )
