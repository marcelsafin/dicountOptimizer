...
from ..domain.models import Purchase, ShoppingRecommendation
...
class FormattingInput(BaseModel):
    ...
    purchases: list[Purchase] = Field(
        description="List of recommended purchases",
        min_length=0,
        max_length=100
    )
...
class FormattingOutput(BaseModel):
    ...
    """
    tips: list[str] = Field(
        description="List of helpful, actionable shopping tips",
        min_length=1,
        max_length=10
    )
...
- Reference specific details from their shopping plan"""
    ...
    def _format_shopping_context(self, input_data: FormattingInput) -> str:
        ...
        if input_data.purchases:
            ...
            # Group by meal
            meals = set(p.meal_association for p in input_data.purchases if p.meal_association)
            if meals:
    ...
    def _parse_response(
        ...
        try:
            ...
            if len(tips) < input_data.num_tips:
                logger.warning(
                    "fewer_tips_than_requested",
                    requested=input_data.num_tips,
                    received=len(tips)
                )
