...
class OutputFormatter:
    ...
    def group_by_store_and_day(self, purchases: List[Purchase]) -> Dict[str, Dict[date, List[Purchase]]]:
        """
    ...
    def generate_tips(self, purchases: List[Purchase]) -> List[str]:
        ...
        for purchase, days in time_sensitive[:2]:  # Max 2 time-sensitive tips
            if days == 0:
                tips.append(f"Buy {purchase.product_name} at {purchase.store_name} today - discount expires soon!")
            elif days == 1:
                tips.append(f"Buy {purchase.product_name} at {purchase.store_name} tomorrow - discount expires in 1 day")
            else:
                tips.append(f"Buy {purchase.product_name} at {purchase.store_name} within {days} days for best savings")
        
        # Find organic products with good value
        organic_products = [p for p in purchases if "økologisk" in p.product_name.lower() or "organic" in p.product_name.lower()]
        ...
        if organic_products and len(tips) < 3:
            ...
            if best_organic.savings > 10:  # Only recommend if savings are significant
                tips.append(f"Great organic deal: {best_organic.product_name} saves you {best_organic.savings:.0f} kr!")
        
        # Add store consolidation tip if shopping at multiple stores
        if len(tips) < 3:
            stores = set(p.store_name for p in purchases)
            if len(stores) > 2:
                ...
                tips.append(f"Consider shopping mainly at {main_store[0]} - they have {main_store[1]} of your items")
    ...
    def generate_motivation(self, total_savings: float, time_savings: float) -> str:
        ...
        motivation = f"{savings_msg}{time_msg} Happy shopping!"
        ...
        for store_name in sorted(grouped.keys()):
            ...
            for purchase_day in sorted(days.keys()):
                ...
                for purchase in days[purchase_day]:
                    savings_str = f"(save {purchase.savings:.0f} kr)" if purchase.savings > 0 else ""
                    meal_str = f"for {purchase.meal_association}" if purchase.meal_association else ""
                    output_lines.append(f"    • {purchase.product_name} - {purchase.price:.0f} kr {savings_str} {meal_str}".strip())
