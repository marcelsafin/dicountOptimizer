...
def example_with_context():
    ...
    logger.info(
        "user_action", user_id=12345, action="login", ip_address="192.168.1.1", success=True
    )

    logger.info(
        "api_call", endpoint="/api/discounts", method="GET", status_code=200, duration_ms=123.45
    )
...
def example_with_log_context():
    ...
    with LogContext(correlation_id="req-xyz-789", request_id="http-001", agent="meal_suggester"):
