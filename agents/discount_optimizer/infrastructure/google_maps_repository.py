...
class GoogleMapsRepository:
    ...
    async def geocode_address(self, address: str) -> Location:
        ...
        try:
            ...
            logger.exception("api_timeout", error=str(e), address=address)
            raise APIError(
            ...
            metrics_collector.record_api_failure(
                "google_maps", "/geocode", status_code=status_code, error_type="http_error"
            )
            logger.exception(
                "http_error",
                error=str(e),
            ...
            metrics_collector.record_api_failure(
                "google_maps", "/geocode", error_type=type(e).__name__
            )
            ...
            raise APIError(f"Unexpected error geocoding address: {e!s}") from e
