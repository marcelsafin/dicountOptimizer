...
class Settings(BaseSettings):
    ...
    salling_group_api_key: SecretStr | None = Field(
        default=None, description="Salling Group API key for discount data (optional for testing)"
    )
    ...
    agent_model: str = Field(
        default="gemini-2.0-flash-exp", description="Gemini model name for AI agents"
    )
    ...
    agent_max_tokens: int = Field(
        default=2000, gt=0, le=8192, description="Maximum output tokens per agent response"
    )

    agent_top_p: float = Field(
        default=0.95, ge=0.0, le=1.0, description="Nucleus sampling parameter for agent responses"
    )

    agent_top_k: int = Field(
        default=40, ge=1, le=100, description="Top-k sampling parameter for agent responses"
    )
    ...
    cache_ttl_seconds: int = Field(
        default=3600, gt=0, description="Time-to-live for cached API responses (seconds)"
    )

    api_timeout_seconds: int = Field(
        default=30, gt=0, le=300, description="Timeout for external API calls (seconds)"
    )
    ...
    redis_password: SecretStr | None = Field(
        default=None, description="Redis password (None for no authentication)"
    )

    redis_max_connections: int = Field(
        default=10, gt=0, le=100, description="Maximum Redis connections in the pool"
    )

    redis_socket_timeout: float = Field(
        default=5.0, gt=0.0, le=60.0, description="Redis socket timeout in seconds"
    )

    redis_socket_connect_timeout: float = Field(
        default=5.0, gt=0.0, le=60.0, description="Redis connection timeout in seconds"
    )
    ...
    max_concurrent_requests: int = Field(
        default=10, gt=0, le=100, description="Maximum concurrent HTTP requests"
    )

    max_retries: int = Field(
        default=3, ge=0, le=10, description="Maximum retry attempts for failed API calls"
    )

    retry_min_wait_seconds: float = Field(
        default=1.0, ge=0.1, le=60.0, description="Minimum wait time between retries (seconds)"
    )

    retry_max_wait_seconds: float = Field(
        default=10.0, ge=1.0, le=300.0, description="Maximum wait time between retries (seconds)"
    )
    ...
    enable_ai_meal_suggestions: bool = Field(
        default=True, description="Enable AI-powered meal suggestions using Gemini"
    )

    enable_caching: bool = Field(default=True, description="Enable caching of API responses")

    enable_metrics: bool = Field(
        default=True, description="Enable metrics collection and monitoring"
    )
    ...
    enable_distributed_tracing: bool = Field(
        default=False, description="Enable distributed tracing with correlation IDs"
    )

    enable_async_operations: bool = Field(
        default=True, description="Enable async/await for I/O operations"
    )
    ...
    log_file: str | None = Field(default=None, description="Path to log file (None = stdout only)")
    ...
    log_backup_count: int = Field(
        default=5, ge=0, description="Number of rotated log files to keep"
    )
    ...
    max_stores_per_recommendation: int = Field(
        default=3, gt=0, le=10, description="Maximum number of stores in a shopping recommendation"
    )

    min_discount_percent: float = Field(
        default=10.0, ge=0.0, le=100.0, description="Minimum discount percentage to consider"
    )
