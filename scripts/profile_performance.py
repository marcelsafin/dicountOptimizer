...
async def profile_ingredient_mapping(
    meals: list[str], discounts: list[DiscountItem]
) -> dict[str, Any]:
...
async def profile_full_pipeline(discounts: list[DiscountItem], meals: list[str]) -> dict[str, Any]:
...
def print_report(results: list[dict[str, Any]], workload_name: str):
    ...
    concurrent_result = next(
        (r for r in results if r.get("operation") == "concurrent_requests"), None
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Profile shopping optimizer performance")
    parser.add_argument(
        "--workload",
