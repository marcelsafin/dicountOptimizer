...
def test_input_validation_empty_strings_filtered():
    ...
    input_data = IngredientMappingInput(
        meal_name="Test Meal", ingredients=["pasta", "", "  ", "cheese"], available_products=[]
    )
...
def test_input_validation_match_threshold_range():
    ...
    input_valid = IngredientMappingInput(
        meal_name="Test", ingredients=["pasta"], available_products=[], match_threshold=0.7
    )
    ...
    with pytest.raises(Exception):  # Pydantic ValidationError
        IngredientMappingInput(
            meal_name="Test", ingredients=["pasta"], available_products=[], match_threshold=1.5
        )
...
async def test_map_ingredients_parses_valid_json(
    monkeypatch, basic_input: IngredientMappingInput, mock_gemini_json_response: str
):
...
async def test_map_ingredients_partial_matches(
    monkeypatch, basic_input: IngredientMappingInput, mock_gemini_partial_response: str
):
...
@pytest.mark.asyncio
async def test_fallback_on_api_error(monkeypatch, basic_input: IngredientMappingInput):
...
@pytest.mark.asyncio
async def test_coverage_calculation_full(monkeypatch, mock_gemini_json_response: str):
...
@pytest.mark.asyncio
async def test_multi_language_mapping(monkeypatch, mock_gemini_json_response: str):
