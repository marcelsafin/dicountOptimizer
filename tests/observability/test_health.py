...
class TestHealthCheckIntegration:
    ...
    async def test_health_check_exception_handling(
        ...
        mock_geocoding_service.health_check = AsyncMock(
    ...
    async def test_health_check_timeout_handling(
        ...
        try:
            await asyncio.wait_for(
                mock_geocoding_service.health_check(),
                timeout=0.1
            )
...
class TestHealthCheckResponseFormat:
    ...
    def test_unhealthy_dependency_response_format(self):
        ...
        assert 'error' in dependency_info['message'].lower() or 'timeout' in dependency_info['message'].lower()
    ...
    def test_degraded_status_response(self):
        ...
        # Count unhealthy
        unhealthy = sum(
            1 for dep in response['dependencies'].values()
