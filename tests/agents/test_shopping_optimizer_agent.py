...
def mock_timeframe():
    ...
    return Timeframe(start_date=date.today(), end_date=date.today() + timedelta(days=7))
...
def mock_recommendation(mock_purchase):
    ...
    return ShoppingRecommendation(
        ...
        total_savings=Decimal("12.50"),
        time_savings=20.0,
        tips=["Shop early in the morning", "Bring reusable bags", "Check expiration dates"],
...
async def test_shopping_optimizer_correlation_id_propagation(
    ...
    # Act
    with patch(
        "agents.discount_optimizer.agents.shopping_optimizer_agent.get_correlation_id"
    ) as mock_get_corr_id:
...
async def test_shopping_optimizer_validation_fails(
    ...
    input_data = ShoppingOptimizerInput(
        address="Invalid Address XYZ", meal_plan=[], timeframe="this week", maximize_savings=True
    )
...
async def test_shopping_optimizer_discount_matcher_fails_fallback(
    ...
    """
    # Arrange
    mock_discount_matcher.match_discounts = AsyncMock(
        side_effect=APIError("Salling API unavailable")
    )
...
async def test_shopping_optimizer_service_call_order(
    ...
    meal_output = MealSuggestionOutput(
        suggested_meals=["Taco Tuesday"], reasoning="Test meals", urgency_notes=""
    )
