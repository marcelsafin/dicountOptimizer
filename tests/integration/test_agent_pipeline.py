...
async def test_full_pipeline_with_coordinates(
    shopping_optimizer_agent, mock_input_validator, mock_location, mock_timeframe, mock_preferences
):
...
async def test_validation_error_propagates(
    ...
    input_data = ShoppingOptimizerInput(address="Invalid Address XYZ", meal_plan=["taco"])
...
async def test_api_error_handled_gracefully(
    ...
    input_data = ShoppingOptimizerInput(latitude=55.6761, longitude=12.5683, meal_plan=["taco"])
